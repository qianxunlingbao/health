<template>
	<view class="page-section-spacing">
		<u-navbar :background="background" title-size="44" back-icon-color="white" title="编辑" title-color="white">
		</u-navbar>
		<view class="healthy">
			...
			<view>
				<u-toast ref="uToast" />
				<u-upload ref="uUpload" :action="action" :auto-upload="true" :on-success="uploadSuccess"></u-upload>
...
<script>
	...
	export default {
		...
		mounted() {
			//请求关系接口
			request({
				url: `cpi/dictDetails/relationship/get`,
				method: 'GET',
				success: (res) => {
			...
			//请求民族接口
			request({
				url: `cpi/dictDetails/nation/get`,
				method: 'GET',
				success: (res) => {
					if (res.statusCode === 200) {
			...
			//请求婚姻状况接口
			request({
				url: `cpi/dictDetails/marital_status/get`,
				method: 'GET',
				success: (res) => {
					if (res.statusCode === 200) {
			...
			//请求老人类型接口
			request({
				url: `cpi/dictDetails/elder_type/get`,
				method: 'GET',
				success: (res) => {
					if (res.statusCode === 200) {
			...
			//请求省市区接口
			request({
				url: `cpi/elderInfo/json/district`,
				method: 'GET',
				success: (res) => {
					if (res.statusCode === 200) {
			...
			//请求居住情况接口
			request({
				url: `cpi/dictDetails/living_situation/get`,
				method: 'GET',
				success: (res) => {
					if (res.statusCode === 200) {
			...
			this.form = this.messageData
			this.formList = this.form
			this.form.ssq = this.messageData.province + " " + this.messageData.city + " " + this.messageData.county
			this.formList.id = this.messageData.id
